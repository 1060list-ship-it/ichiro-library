...
**Server Action の競合制御**：playlist 編集 Action は操作種別（追加 / 削除 / 並び替え / title 更新）ごとに独立した Action とし、各 Action は `playlist_id` + `updated_at`（楽観ロック）を受け取る。`updated_at` が DB と一致しない場合は `409 Conflict` を返しクライアント側でリロードを促す。同一 position への並行ドロップは `UNIQUE(playlist_id, position)` 違反でクライアント再試行（詳細はセクション3「並行書き込みの衝突ケース」参照）。title / description の保存は **blur イベント**で発火（debounce ではなく明示的フォーカス離脱時）。
