...
**ステータス**: Codex 第2回6エージェント検証済み・修正版（2026-06-21）
...
const getCurrentUserRole = cache(async () => {
  const session = await verifySession()
  // user_roles テーブルから取得（JWT クレームは使用しない）
  return userRole
})
...
**リバランス条件**：Server Action は並び替え時に隣接 position の gap を計算し、gap が `0.00000002` 以下（最小単位の2倍）になった場合にリバランスを実行する（`0.00000001` ちょうどで判定すると丸め誤差で既存 position と衝突するため early trigger）。リバランスは同一 `playlist_id` の全行を `10000, 20000, 30000...` に整数リセットしてから更新する（transaction 内）。リバランス自体は稀で、一般ユーザーには不可視。
...
| `GET /admin` | 302 → `/login` | 302 → `/login` | 200 | proxy.ts redirect |
