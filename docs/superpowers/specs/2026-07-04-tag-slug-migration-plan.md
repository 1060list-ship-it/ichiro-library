...
- 一連の障害（Macスリープハング×2回、Gemini月間支出上限到達、chapters空スキップによる永久未処理64件）を解決：
  - `summarize.py`にGemini呼び出しタイムアウト180秒を追加（コミット`2f2b33f`。ただしMacスリープによるプロセス凍結には無効と後日判明、タイムアウトは主にネットワーク不安定時のハング対策として有効）
  - `reprocess_videos.py`のchapters空スキップ問題を修正（コミット`fcfbf0d`）：chapters=[]でもsummary/tags/highlights等を保存しai_prompt_ver=v4を刻印するよう変更。kanaレビューで実データ検証し、該当64件中62件がこのパターンと確認済み
  - Gemini月間支出上限は一幾がAI Studio（[https://ai.studio/spend](https://ai.studio/spend)）で確認・引き上げ
  - Macスリープ対策は「蓋を開けたままの時間帯にまとめて実行する」運用でカバー（`caffeinate -i`はクラムシェルスリープを防げないため技術的な完全解決はしていない）
- 最終結果（2026-07-11時点）：**307件中306件がv4処理完了**。残り1件は字幕なし（transcript_failed、処理対象外）
- summary_failed・v3残りともに0件
- 次アクション：P6（事後検証）へ進む

#### 2026-07-05 再開試行・IPブロック未解消のため再中断

- `veehz1tofnc`（前回IPブロックが発生していた動画）で単発テスト実行→**14時間経過後もIPブロック継続**（60秒→120秒→240秒のリトライを経て失敗）。ただしフォールバック（Supadata）で最終的に字幕取得・Gemini呼び出しは成功（chapters=0のため刻印はスキップ、既存の安全設計）
- DB書き込みは発生せず（`ai_prompt_ver='v3'`のまま）、安全に中断できることを確認
- **残り73件（v3/public）のtranscript_snapshot有無を調査**: snapshot既存27件（通常45秒/件で処理可）・**snapshot無し46件（新規フェッチが必要でIPブロックの影響を受ける）**
- 概算所要時間: snapshot既存27件で約20分＋snapshot無し46件で1件約8分×46≈6.1時間＝**合計約6.5時間見込み**（IPブロックが今の状態のまま続いた場合）
- 一幾判断：本日は中断し、IPブロック解消を別日に確認してから再開する方針に決定
- 次回再開時は、まず1件（`veehz1tofnc`等snapshot無し動画）でIPブロック解消を確認してから本再開すること
